...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    if (!email || !password || !partner1 || !partner2) {
  ...
  if (success) return (
    <>
      ...
      <main className="max-w-md mx-auto px-6 py-20 text-center">
        ...
        <p style={{color:'#6B7280'}}>We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account and start planning.</p>
      </main>
    </>
  );
    ...
    <>
      ...
      <main className="max-w-md mx-auto px-6 py-12">
        <div className="text-center mb-8">
          ...
          <h1 className="font-serif text-3xl font-bold mb-2" style={{color:'#2C2C3E'}}>Start Planning Your Day</h1>
          <p style={{color:'#6B7280'}}>Create your free wedding planning account</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-8" style={{border:'1px solid #E8DDD8'}}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2" style={{color:'#475569'}}>Partner 1 Name *</label>
                <input value={partner1} onChange={e => setPartner1(e.target.value)} placeholder="e.g. Sarah" className="w-full h-12 px-4 rounded-xl outline-none" style={{border:'1px solid #E8DDD8', background:'#F8FAFC', color:'#2C2C3E'}} />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2" style={{color:'#475569'}}>Partner 2 Name *</label>
                <input value={partner2} onChange={e => setPartner2(e.target.value)} placeholder="e.g. James" className="w-full h-12 px-4 rounded-xl outline-none" style={{border:'1px solid #E8DDD8', background:'#F8FAFC', color:'#2C2C3E'}} />
              </div>
            </div>

            <div>
              <label className="block text-sm font-semibold mb-2" style={{color:'#475569'}}>Wedding Date (optional)</label>
